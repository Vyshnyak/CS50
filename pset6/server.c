...
bool load(FILE* file, BYTE** content, size_t* length)
{
    ...
    while(fread(buffer + size, 1, 1, file) != 0) {
        size++;
...
bool parse(const char* line, char* abs_path, char* query)
{
    ...
    char req_trg[last_SP - first_SP];
    char http_ver[end - last_SP];

    // copy method substring from lne to corresponding array
    strncpy(method, line, first_SP - line);
    method[first_SP - line] = '\0';

    // copy req_trg substring from line to corresponding array
    strncpy(req_trg, first_SP + 1, last_SP - first_SP - 1);
    req_trg[last_SP - first_SP - 1] = '\0';

    // copy http_ver substring from line to corresponding array
    strncpy(http_ver, last_SP + 1, end - last_SP - 1);
    http_ver[end - last_SP - 1] = '\0';

    // ensure line is consistent to these rules
    ...
    // pointer to ?
    char* query_start = strchr(req_trg, '?');

    // split request line to abs_path and query and write to array
    if (query_start != NULL) {
        const char* abs_path_copy = strtok(req_trg, "?");
